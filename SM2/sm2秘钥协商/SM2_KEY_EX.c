...
ZB[], unsigned char K[], int klen, epoint* RB, epoint* V, unsigned char hash[])
{
    ...
    printf(" 计算[x1_]RA=(x,y)：");
    printf("\n 坐标x：");
    cotnum(V->X, stdout);
    printf(" 坐标y：");
    cotnum(V->Y, stdout);
    // 计算椭圆曲线点PA+[x1_]Ra=(x,y)
    ecurve_add(PA, V);      //  V=PA+V
    epoint_get(V, temp_x, temp_y);
    printf(" 计算PA+[x1_]Ra=(x,y)：");
    printf("\n 坐标x：");
    cotnum(V->X, stdout);
    printf(" 坐标y：");
    cotnum(V->Y, stdout);
    // 计算椭圆曲线点[h*tb](PA+[x1_]Ra)=(x,y)
    ...
    SM3_done(&md, hash);
    printf("\n 计算SB：");
    for (int i = 0; i < SM2_NUMWORD * 2 + SM3_len / 4; i++)
    {
        printf("%X", hash[i]);
    }
    return 0;
...
int main()
{
    ...
    printf("使用说明：本程序是SM2密钥协商算法演示程序\n");
    printf("**********************$声明信息$******************************\n");
    printf("================SM2密钥协商算法程序演示================\n\n");
