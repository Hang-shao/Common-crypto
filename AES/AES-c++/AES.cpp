...
unsigned char *AES::EncryptECB(const unsigned char in[], unsigned int inLen,
  ...
  KeyExpansion(key, roundKeys);//密钥编排，轮密钥存放在roundKeys
...
unsigned char *AES::DecryptECB(const unsigned char in[], unsigned int inLen,
  ...
  unsigned char *out = new unsigned char[inLen];
  unsigned char *roundKeys = new unsigned char[4 * Nb * (Nr + 1)];
  KeyExpansion(key, roundKeys);//密钥扩展算法
  for (unsigned int i = 0; i < inLen; i += blockBytesLen) {
...
unsigned char *AES::EncryptCBC(const unsigned char in[], unsigned int inLen,
  ...
  for (unsigned int i = 0; i < inLen; i += blockBytesLen) {
    XorBlocks(block, in + i, block, blockBytesLen); //初始化向量先与明文分组异或
    EncryptBlock(block, out + i, roundKeys);        //然后加密
    memcpy(block, out + i, blockBytesLen);          //将输出的密文赋值为初始化向量，用于下次加密
  }
...
unsigned char *AES::EncryptCFB(const unsigned char in[], unsigned int inLen,
  ...
  unsigned char *roundKeys = new unsigned char[4 * Nb * (Nr + 1)];
  KeyExpansion(key, roundKeys);
  memcpy(block, iv, blockBytesLen);     //初始化向量赋值为block
  for (unsigned int i = 0; i < inLen; i += blockBytesLen) {
    EncryptBlock(block, encryptedBlock, roundKeys);         //先加密初始化向量（block -》encryptedBlock）
    XorBlocks(in + i, encryptedBlock, out + i, blockBytesLen);      //然后与明文分组异或
    memcpy(block, out + i, blockBytesLen);                  //最后将密文赋值为block，用于下一次加密
  }
...
void AES::EncryptBlock(const unsigned char in[], unsigned char out[],
  ...
  unsigned char state[4][Nb];//状态矩阵（4行，Nb列）
  unsigned int i, j, round;

  for (i = 0; i < 4; i++) { //将输入的分组128bit存放在状态矩阵中
    for (j = 0; j < Nb; j++) {
      state[i][j] = in[i + 4 * j];
    }
  }
...
void AES::DecryptBlock(const unsigned char in[], unsigned char out[],
  ...
  AddRoundKey(state, roundKeys + Nr * 4 * Nb);//轮密钥加

  //前9轮
  for (round = Nr - 1; round >= 1; round--) {
    InvSubBytes(state);//逆比特转换
...
void AES::SubBytes(unsigned char state[4][Nb]) {
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < Nb; j++) {
      t = state[i][j];
      state[i][j] = sbox[t / 16][t % 16];//前4位位行，后4位为列
    }
  }
}
...
void AES::AddRoundKey(unsigned char state[4][Nb], unsigned char *key) {
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < Nb; j++) {
      state[i][j] = state[i][j] ^ key[i + 4 * j];//状态矩阵与轮密钥逐比特异或
    }
  }
}
